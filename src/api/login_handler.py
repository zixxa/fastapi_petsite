...
async def _get_user_by_email_for_auth(email: str, db: AsyncSession):
    async with db as session:
        async with session.begin():
            ...
            return user_dal.get_user_by_email(email=email)
...
async def get_current_user_from_token(
    ...
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED, detail="Could not validate credential"
    )
    try:
        payload = jwt.decode(token, settings.SECRET_KEY, algorithms=Depends(get_db))
